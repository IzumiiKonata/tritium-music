...
import tritium.music.repackage.com.jsyn.ports.QueueDataCommand;
...
import tritium.music.repackage.com.jsyn.unitgen.VariableRateDataReader;
...
public class AudioSample extends SoundObject {
    ...
    public VariableRateDataReader player;
    ...
    public AudioSample(int frames) {
        this(frames, false);
    }

    public AudioSample(int frames, boolean stereo) {
        this(frames, stereo, 44100); // read current framerate from Engine instead?
    }
     ...
     */
    public AudioSample(int frames, boolean stereo, int frameRate) {
        super();
    ...
    public AudioSample(int frames, int frameRate) {
        this(frames, false, frameRate);
    }
    ...
    /**
     * @param data an array of float values to be used as this audiosample's sound
     ...
     *             <b>[-0.5, 0.5]</b>
     */
    public AudioSample(float[] data) {
        this(data, false);
    }

    public AudioSample(float[] data, boolean stereo) {
        this(data, stereo, 44100); // read current framerate from Engine instead?
    }

    public AudioSample(float[] data, int frameRate) {
        this(data, false, frameRate);
    }

    public AudioSample(float[] data, boolean stereo, int frameRate) {
        super();
     ...
     *
     * @param time position in the audiosample that the next playback should start
     *             from, in seconds.
     * @webref Sampling:AudioSample
     ...
     *
     * @param frameNumber number to jump to.
     * @webref Sampling:AudioSample
	...
	*/
     ...
     *
     * @param rate relative playback rate to use. 1 is the original speed. 0.5 is
     *             half speed and one octave down. 2 is double the speed and one
     *             octave up.
     * @param pos  the panoramic position of this sound unit from -1.0 (left) to 1.0
     *             (right). Only works for mono audiosamples!
     * @param amp  the desired playback amplitude of the audiosample as a value from
...
//	}
    ...
    /**
     * @nowebref public void play(float rate, float pos, float amp, float add) {
    this.add(add);
     ...
     * @param pos  the panoramic position of this sound unit from -1.0 (left) to 1.0
     *             (right). Only works for mono audiosamples!
     * @param cue  position in the audiosample that playback should start from, in
     *             seconds.
     * @webref Sampling:AudioSample
     * @webBrief Starts the playback of the audiosample.
     **/
     ...
     *
     * @param duration duration that this audiosample should be played back for, in seconds.
     ...
     *
     * @param data the target array that the read data is written to
     */
     ...
     *
     * @param startFrame the index of the first frame of the audiosample that should be
     *                   read
     * @param startIndex the position in the array where the first read frame should be
     *                   written to (typically 0)
     * @param numFrames  the number of frames that should be read (can't be greater than
    ...
    /**
     * @param frameIndex the index of the single frame of the audiosample that should be
    ...
    /**
     * @param channelIndex the channel from which to extract the frame value (0 for left,
     ...
     *
     * @param data the array from which the sample data, up to
     ...
     * @param startFrame the index of the first frame of the audiosample that should be
     *                   written to
     * @param startIndex the position in the array that the first value to write should be
     *                   taken from (typically 0)
     * @param numFrames  the number of frames that should be written (can't be greater than
     *                   audiosample.channels() * data.length - startIndex)
     * @webref Sampling:AudioSample
    ...
    /**
     * @param index the index of the single frame of the audiosample that should be
     *              set to the given value
     * @param value the float value that the given audio frame should be set to
     */
